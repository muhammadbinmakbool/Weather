...
const Task1 = () => {
  ...
  const newContacts = useCallback(
    async (name, number, index) => {
      ...
      setName('');
      setNumber('');
      await AsyncStorage.setItem(index.toString(), JSON.stringify(newContact));
    },
  ...
  useEffect(() => {
    const RetrievedContact = AsyncStorage.getItem('contacts');
    const ParsedData = JSON.parse(RetrievedContact);
    setSearchList([...contacts, ...ParsedData]);
  }, [contacts]);
  ...
  return (
    <ScrollView style={{margin: 10}}>
      ...
      <TouchableOpacity
        ...
        onPress={() => newContacts(name, number, index)}>
        <Text
          style={{
            textAlign: 'center',
            color: 'white',
